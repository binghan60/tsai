        ...
        <li v-for="record in pet.medicalRecords" :key="record._id">
          ...
          <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <div class="flex flex-wrap items-center gap-1 text-sm">
              ...
              <button type="button" :disabled="deletingRecordId === record._id" class="inline-flex min-h-11 items-center gap-1 rounded-xl px-2.5 font-medium text-destructive hover:bg-destructive/10 disabled:opacity-50" :aria-label="`刪除健檢紀錄 ${formatDate(record.visitDate)}`" @click="openRemoveRecord(record)"><Trash2 class="h-4 w-4" />刪除</button>
            </div>
          </div>
          </Card>
        </li>
