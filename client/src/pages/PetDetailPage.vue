        ...
        <li v-for="record in pet.medicalRecords" :key="record._id">
          ...
          <div class="flex flex-wrap items-start justify-between gap-3">
            ...
            <div class="flex flex-wrap items-center gap-1 text-sm">
              ...
              <button v-if="isFinalizedRecord(record) && !isShareActive(record)" type="button" :disabled="sharingId === record._id" class="inline-flex min-h-11 items-center gap-1.5 rounded-xl px-3 font-medium text-belle-600 hover:bg-belle-50 disabled:opacity-50 dark:text-brand-400 dark:hover:bg-brand-500/10" @click="shareRecord(record)"><Share2 class="h-4 w-4" />{{ sharingId === record._id ? '處理中…' : '分享' }}</button>
