  ...
  <div class="space-y-5 pb-8">
    <header class="flex flex-wrap items-center gap-4">
      ...
      <div>
        ...
        <p class="mt-0.5 text-xs text-muted-foreground">核准後才建立正式飼主與寵物資料</p>
      </div>
      <div class="ml-auto flex gap-2">
        <Button variant="secondary" size="sm" as-child
          ><router-link to="/reception"><ArrowLeft class="h-4 w-4" />返回櫃台</router-link></Button
        ><Button variant="secondary" size="sm" :disabled="loading" @click="refresh"><RefreshCw class="h-4 w-4" />重新載入</Button>
      </div>
    </header>
    ...
    <section v-if="!loading && !selectedId" class="overflow-hidden rounded-xl border border-border bg-card">
      <div class="flex items-center gap-3 border-b border-border px-5 py-3">
        <h2 class="text-base font-semibold">待審核</h2>
      ...
      <button v-for="item in items" :key="item._id" type="button" class="flex w-full flex-wrap items-center gap-4 border-b border-border px-5 py-4 text-left last:border-b-0 hover:bg-field" @click="selectedId = item._id">
        ...
        <span class="min-w-0 flex-1"
          ...
          ><span class="mt-0.5 block truncate text-sm text-muted-foreground">{{ item.owner.name }} · {{ item.owner.phone }}</span></span
        >
    ...
    <section v-else-if="!loading" class="paper-detail-shell">
      <div class="mb-3 flex items-center justify-between">
        <Button variant="secondary" size="sm" @click="selectedId = ''"><ArrowLeft class="h-4 w-4" />返回列表</Button><span class="text-xs text-muted-foreground">完整初診表</span>
      </div>
      <article v-for="item in items.filter((item) => item._id === selectedId)" :key="item._id" class="paper-form">
        <div class="paper-header">
          <div>
            <p class="paper-logo">ROYAL CANIN</p>
            <h3>初診掛號單</h3>
          </div>
        ...
        <section class="paper-section">
          ...
          <div class="paper-grid">
            ...
            <div>
              <p class="paper-subtitle">生活狀況</p>
              <p class="paper-line"><b>家中貓口：</b>{{ item.pet.householdCatCount ?? '未填寫' }}<template v-if="item.pet.householdCatCount !== null"> 隻</template></p>
              <p class="paper-line">
                <b>主餐配菜：</b>{{ item.pet.foods?.join('、') || '未填寫' }}<template v-if="item.pet.foodsOther">（{{ item.pet.foodsOther }}）</template>
              </p>
              <p class="paper-line"><b>放飯頻率：</b>{{ { free: '任食', scheduled: `定食定量${item.pet.mealsPerDay ? `：一日 ${item.pet.mealsPerDay} 餐` : ''}` }[item.pet.feedingType] || '未填寫' }}</p>
            </div>
          </div>
