...
import { Mail, Pencil, Phone, Search, Trash2, Users } from '@lucide/vue';
...
<template>
  <section class="mx-auto max-w-7xl space-y-5">
    ...
    <div class="rounded-2xl border border-cream-300 bg-cream-50 p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:shadow-none">
      <label for="owner-search" class="mb-2 block text-xs font-medium text-ink-500 dark:text-zinc-400">搜尋飼主</label>
      <div class="relative">
        ...
        <input id="owner-search" v-model="query" type="search" autocomplete="off" placeholder="輸入姓名或電話" class="w-full rounded-xl border border-cream-300 bg-white py-3 pl-11 pr-4 text-sm text-ink-900 placeholder:text-ink-400 focus:border-belle-500 focus:outline-none focus:ring-2 focus:ring-belle-100 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-500 dark:focus:ring-brand-500/20" />
      </div>
    </div>
