  ...
  <div class="flex flex-col gap-4">
    <header class="flex flex-wrap items-center gap-4">
      ...
      <div class="ml-auto flex flex-wrap items-center gap-2">
        ...
        <span class="ml-2 inline-flex items-center gap-1.5 text-xs text-muted-foreground"> <span class="h-1.5 w-1.5 rounded-full" :class="connected ? 'bg-success' : 'bg-warning'"></span>{{ connected ? '即時同步' : '重新連線中' }} </span>
      </div>
    </header>
    ...
    <div class="grid gap-4 xl:grid-cols-[24rem_minmax(0,1fr)]">
      <section class="flex h-full flex-col rounded-xl border border-border bg-card" aria-label="候診佇列">
          ...
          <template v-else>
            ...
            >
              <button type="button" class="group flex w-full cursor-pointer items-center gap-2 rounded-lg px-2 py-2 text-left text-xs font-semibold text-muted-foreground transition-colors hover:bg-field hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 active:bg-accent" :aria-expanded="!collapsedGroups[group.key]" @click="toggleGroup(group.key)">
                ...
                ><span class="font-normal">{{ group.list.length }}</span>
              </button>
              <Transition name="queue-collapse">
                <div v-if="!collapsedGroups[group.key]" class="overflow-hidden">
                  <div v-for="item in group.list" :key="item._id" class="mb-2 cursor-pointer rounded-xl border p-3 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50" :class="String(item._id) === activeId ? 'border-primary/40 bg-accent' : 'border-border bg-card hover:bg-field'" role="button" tabindex="0" @click="openPatient(item)" @keydown.enter.prevent="openPatient(item)" @keydown.space.prevent="openPatient(item)">
                    <div class="flex items-center gap-2">
                      <div class="flex min-w-0 flex-1 items-center gap-2">
                        <span v-if="item.checkinNumber != null" class="inline-flex h-6 min-w-6 shrink-0 items-center justify-center rounded-full px-1 text-xs font-semibold tabular-nums" :class="String(item._id) === activeId ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground'">{{ item.checkinNumber }}</span>
            ...
            <Transition name="queue-collapse">
              <div v-if="!collapsedGroups.completed" class="overflow-hidden">
                <button v-for="item in finished" :key="item._id" type="button" class="mb-1 flex w-full items-center gap-3 rounded-xl px-2.5 py-2 text-left hover:bg-field" @click="openPatient(item)">
      ...
      <section class="rounded-xl border border-border bg-card" aria-label="看診工作區">
        <div v-if="openTabs.length" class="flex shrink-0 items-center gap-2 overflow-x-auto border-b border-border bg-field/40 px-4 py-2">
          <div v-for="tab in openTabs" :key="tab._id" class="flex h-10 shrink-0 items-center gap-2 rounded-lg px-3 text-sm font-medium" :class="String(tab._id) === activeId ? 'bg-card text-foreground shadow-sm' : 'bg-secondary text-secondary-foreground'">
        ...
        <VisitWorkspace v-for="tab in openTabs" v-show="String(tab._id) === activeId" :key="tab._id" :appointment="tab" :templates="templates" @updated="onWorkspaceUpdate" @open-record="(appointment) => router.push({ path: `/records/${appointment.recordId}/edit`, query: { visit: appointment._id, visitDate: appointment.date } })" />
        <EmptyState v-if="!active && !loading" :icon="CalendarClock" title="從左邊選一位病患開始看診" description="選取病患後，按「開始看診」才會記錄開始時間。可以同時開好幾位，切換不會清空已輸入的內容。" inset />
      </section>
    </div>
  </div>
</template>
