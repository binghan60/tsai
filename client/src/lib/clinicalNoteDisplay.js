...
// 報告的每一列：{ key, label, text, tone, emphasis }；量測併成一列「生命徵象」，用 items 各自呈現。
...
// 表單欄位定義，順序＝畫面順序。multiline 決定用 Textarea 還是 Input。
const APPOINTMENT_EDIT_FIELDS = [
  ...
  { key: 'visitNote', label: '本次簡易紀錄', multiline: true, rows: 5, maxlength: 10000 },
...
const MEDICATION_EDIT_FIELDS = [
  { key: 'condition', label: '病況', multiline: true, rows: 3, maxlength: 5000 },
  { key: 'prescription', label: '藥單', multiline: true, rows: 4, maxlength: 10000, required: true },
  { key: 'note', label: '備註', multiline: true, rows: 2, maxlength: 3000 },
]
...
export function journalEditError(note, form) {
  ...
  for (const field of fields) {
    const value = form[field.key]
    if (field.required && blank(value)) return `請填寫${field.label}`
    if (field.numeric && !blank(value) && (!Number.isFinite(Number(value)) || Number(value) < 0)) return `${field.label}必須是有效的非負數`
    if (field.maxlength && String(value ?? '').trim().length > field.maxlength) return `${field.label}最多 ${field.maxlength} 字`
  }
  if (fields === APPOINTMENT_EDIT_FIELDS && fields.every((field) => blank(form[field.key]))) return '日誌內容不能全部清空'
  return ''
}
