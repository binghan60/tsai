...
<template>
  <aside class="space-y-3 xl:sticky xl:top-3">
    <section class="rounded-2xl border border-border bg-card p-4 shadow-sm dark:shadow-none">
      ...
      <div v-if="queue.length" class="mt-3 divide-y divide-border">
        <div v-for="appointment in queue" :key="appointment._id" class="flex items-center gap-3 py-2 first:pt-0 last:pb-0">
          <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-sm font-bold tabular-nums text-primary">
      ...
      <div v-else class="mt-3 rounded-xl border border-dashed border-border bg-muted/30 px-3 py-4 text-center">
    ...
    <section class="rounded-2xl border border-border bg-card p-4 shadow-sm dark:shadow-none">
      ...
      <dl class="mt-3 grid grid-cols-2 gap-2">
        ...
        <div class="rounded-xl bg-muted/45 p-2.5">
          ...
          <dd class="mt-0.5 text-xl font-semibold tabular-nums text-foreground">{{ counts.cancelled || 0 }}</dd>
        </div>
      </dl>
    </section>
  </aside>
