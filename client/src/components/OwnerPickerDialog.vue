...
<template>
  <ModalDialog v-if="open" size="md" @close="close">
    <div class="p-6 pb-3 sm:p-7 sm:pb-3">
      <DialogTitle>選擇既有飼主</DialogTitle>
      <DialogDescription class="mt-1">選擇飼主，為其登記另一隻新寵物。</DialogDescription>
    </div>
    ...
    <div class="space-y-3 px-6 pb-6 sm:px-7 sm:pb-7">
      <div class="relative">
        ...
        <Input v-model="query" type="text" inputmode="search" class="h-11 pl-10 pr-10" placeholder="搜尋飼主姓名或電話" aria-label="搜尋飼主姓名或電話" />
        ...
        >
          ...
          <template #description>
            <span class="flex items-center gap-1 truncate">
              <User class="h-3.5 w-3.5 shrink-0" />{{ owner.phone || '未填寫電話' }}
            </span>
          </template>
        </PickerOptionRow>
      </div>
      <p v-else class="rounded-xl border border-border px-4 py-8 text-center text-sm text-muted-foreground">{{ query.trim() ? '找不到符合的飼主。' : '目前沒有可選擇的飼主。' }}</p>
