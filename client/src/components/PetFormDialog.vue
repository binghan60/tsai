      ...
      <div class="space-y-6 p-6 pt-3 sm:p-7 sm:pt-3">
        ...
        <div class="space-y-3.5">
          <div class="grid gap-x-4 gap-y-4 sm:grid-cols-2">
            ...
            <div class="space-y-1.5">
              <Label for="pet-breed" class="text-xs font-medium text-foreground">品種</Label>
              <Input id="pet-breed" v-model="breed" class="border-border" placeholder="例：米克斯、美短" />
            </div>
            <div class="space-y-1.5">
              <Label for="pet-birth-date" class="text-xs font-medium text-foreground">生日</Label>
              <Input id="pet-birth-date" v-model="birthDate" type="date" class="border-border" />
            </div>
            <div class="space-y-1.5">
              <Label for="pet-sex" class="text-xs font-medium text-foreground">性別</Label>
              <Select v-model="sex">
                <SelectTrigger id="pet-sex" class="w-full border-border"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div class="space-y-1.5">
              <Label for="pet-neutered" class="text-xs font-medium text-foreground">絕育狀態</Label>
              <Select v-model="neutered">
                <SelectTrigger id="pet-neutered" class="w-full border-border"><SelectValue /></SelectTrigger>
                <SelectContent>
        ...
        <div class="space-y-4 border-t border-border pt-5">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="space-y-1.5">
              <Label for="pet-allergies" class="text-xs font-medium text-foreground">過敏紀錄</Label>
              <Textarea id="pet-allergies" v-model="allergies" rows="2" class="border-border" placeholder="例：對某類抗生素過敏" />
            </div>
            <div class="space-y-1.5">
              <Label for="pet-chronic" class="text-xs font-medium text-foreground">慢性病／重要病史</Label>
              <Textarea id="pet-chronic" v-model="chronicConditions" rows="2" class="border-border" placeholder="例：慢性腎臟病二期" />
            </div>
            <div class="space-y-1.5">
              <Label for="pet-medications" class="text-xs font-medium text-foreground">目前用藥</Label>
              <Textarea id="pet-medications" v-model="currentMedications" rows="2" class="border-border" placeholder="例：每日降血壓藥物" />
            </div>
            <div class="space-y-1.5">
              <Label for="pet-notes" class="text-xs font-medium text-foreground">其他備註</Label>
              <Textarea id="pet-notes" v-model="notes" rows="2" class="border-border" placeholder="例：看診較為緊張" />
            </div>
          </div>
        </div>
