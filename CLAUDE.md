...
| `/pets`、`/pets/:id` | 寵物列表／詳情 | **飼主不是獨立可瀏覽的實體**——沒有 `/owners` 或 `/owners/:id`，飼主資料一律以附帶資訊的形式跟著寵物出現。詳情頁把寵物資料卡與飼主資料卡並排（`pets.ownerId` populate 出 `name/phone/email/address/__v`），兩張卡各自「編輯」後直接就地變成輸入框、儲存/取消，不彈 Modal——這頁的定位就是報到時一眼同時看到兩邊，彈窗等於又把兩份資料拆回兩個畫面。也含病歷日誌（隨手記事，見第二節 `clinicalNotes`）與歷次報告 |
