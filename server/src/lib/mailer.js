...
export async function sendHealthReportEmail({
  ...
  const fromName = process.env.MAIL_FROM_NAME?.trim() || '寵物健康管理系統';
  const safePetName = petName || '您的寵物';
  const greeting = ownerName ? `${ownerName} 您好：` : '您好：';
  const subject = `${safePetName}的健檢報告`;
  const text = `${greeting}\n\n${safePetName}的健檢報告已完成，PDF報告已附在本封郵件中。\n\n也可以透過以下無期限連結查看：\n${reportUrl}\n\n若需停止分享，請聯絡醫院撤銷連結。`;
  const html = `
    <p>${escapeHtml(greeting)}</p>
    <p>${escapeHtml(safePetName)}的健檢報告已完成，PDF報告已附在本封郵件中。</p>
    <p>也可以透過以下無期限連結查看：</p>
    <p><a href="${escapeHtml(reportUrl)}">開啟健檢報告</a></p>
    <p style="color:#666;font-size:13px">若需停止分享，請聯絡醫院撤銷連結。</p>
  `;
